...
def get_filter_repo_commands(filename="input.txt"):
    ...
    try:
        ...
        import posixpath
        glob_pattern = posixpath.join("**", filename)
        result = subprocess.run(
            ["git", "ls-files", glob_pattern, filename],
            capture_output=True,
            text=True,
            check=True
        )
        ...
        result = subprocess.run([
            "git", "log", "--all", "--name-only", "--pretty=format:", "--", glob_pattern, filename
        ...
        # Use posixpath for consistent forward slashes regardless of OS
        import posixpath
        glob_pattern = posixpath.join("**", filename)
        filter_commands = [
            ["git", "filter-repo", "--path-glob", glob_pattern, "--invert-paths", "--force"],
...
def backup_warning(filename="input.txt"):
    ...
    print()
    preserve_response = input("Do you want to keep the files on disk (remove only from Git history)? (yes/no): ").lower().strip()
    preserve_files = preserve_response in ['yes', 'y']
    
    if preserve_files:
        print("Files will be preserved on disk after removal from Git history.")
    else:
        print("Files will be completely removed from both Git history and disk.")
    
    response = input("\nDo you want to continue? (yes/no): ").lower().strip()
    return response in ['yes', 'y'], historical_files, filter_commands, preserve_files


def remove_input_files(historical_files, filter_commands, filename="input.txt", preserve_files=True):
    ...
    try:
        ...
        # Capture original remote URL before git filter-repo removes it
        original_remote_url = None
        try:
            result = subprocess.run(["git", "remote", "get-url", "origin"], capture_output=True, text=True, check=True)
            original_remote_url = result.stdout.strip()
        except subprocess.CalledProcessError:
            # No remote or origin doesn't exist
            pass
        
        # Backup files if preserve_files is True
        backup_dir = None
        if preserve_files:
            import shutil
            import tempfile
            import time
            import hashlib
            # Create unique prefix with timestamp hash
            timestamp_hash = hashlib.md5(str(time.time()).encode()).hexdigest()[:6]
            backup_dir = tempfile.mkdtemp(prefix=f"git_filter_backup_{timestamp_hash}_")
            print(f"\nBacking up current {filename} files to: {backup_dir}")
            
            # Get current files that exist
            result = subprocess.run(["git", "ls-files"], capture_output=True, text=True, check=True)
            current_git_files = result.stdout.splitlines()
            
            backed_up_count = 0
            for git_file in current_git_files:
                if git_file.endswith(filename) and Path(git_file).exists():
                    backup_path = Path(backup_dir) / git_file
                    backup_path.parent.mkdir(parents=True, exist_ok=True)
                    shutil.copy2(git_file, backup_path)
                    backed_up_count += 1
            
            print(f"Backed up {backed_up_count} {filename} files")
        ...
        if preserve_files and backup_dir:
            print(f"\nRestoring {filename} files to working directory...")
            import shutil
            restored_count = 0
            for backup_file in Path(backup_dir).rglob(filename):
            ...
            print(f"Restored {restored_count} {filename} files to working directory")
        ...
        print("\n" + "="*50)
        print("NEXT STEPS - Complete the process:")
        print("="*50)
        print("\n1. Re-add your Git remote:")
        if original_remote_url:
            print(f"   git remote add origin {original_remote_url}")
        else:
        ...
        print("   OR (if --force-with-lease fails):")
        print("   git push --force origin main")
        print("\n3. Verify the push worked:")
        print("   git remote -v")
        print("   git status")
        print("\nNote: This operation has rewritten Git history.")
        print("All collaborators will need to re-clone or reset their local repos.")
...
def main():
    ...
    should_continue, historical_files, filter_commands, preserve_files = backup_warning(filename)
    if not should_continue:
        print("Operation cancelled.")
        sys.exit(0)
    
    # Perform the operation using the exact same data
    if remove_input_files(historical_files, filter_commands, filename, preserve_files):
